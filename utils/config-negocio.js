...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '3b9f10c9-8d4a-4db3-936d-943e0374c1bd'; // ID de Lia Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Lia Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '52583489';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'liamelairiarte@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Lia Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'lianails';
};
...
console.log('✅ config-negocio.js listo para Lia Nails');
